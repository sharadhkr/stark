...
const AdminComboOffer = ({ comboOffers = [], setComboOffers, products = [], setProducts, loading: parentLoading }) => {
  ...
  console.log('ComboOffers prop:', comboOffers); // Debug
  console.log('Products prop:', products); // Debug
  ...
  const checkAdmin = async () => {
    ...
    try {
      ...
      console.log('Verifying admin token:', token); // Debug
      const verifyRes = await axios.get('/api/admin/auth/verify-token');
      console.log('Verify token response:', verifyRes.data); // Debug
      ...
      console.error('Admin verification error:', {
        ...
        token: localStorage.getItem('adminToken') || 'No token',
      });
  ...
  const handleFileChange = (e) => {
    ...
    console.log('Selected files for combo offer:', files); // Debug
  ...
  const handleOfferSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      console.log('Submitting combo offer:', offerForm); // Debug
      let response;
      if (editingOffer) {
      ...
      if (selectedFiles.length > 0) {
        ...
        selectedFiles.forEach((file, index) => {
          ...
          console.log(`Appending file ${index} for combo offer:`, file.name); // Debug
        });
        ...
        console.log('Image upload response:', imageRes.data); // Debug
  ...
  const handleDeleteOffer = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      console.log(`Deleting combo offer ${id}`); // Debug
  ...
  const handleBulkDeleteOffers = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      console.log('Bulk deleting combo offers:', selectedComboOffers); // Debug
  ...
  const handleDeleteImage = async (offerId, index) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      console.log(`Deleting image ${index} for combo offer ${offerId}`); // Debug
      const res = await axios.delete(`/api/admin/auth/combo-offers/${offerId}/image/${index}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log('Image delete response:', res.data); // Debug
  ...
  const handleToggleImage = async (offerId, index) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      console.log(`Toggling image ${index} for combo offer ${offerId}`); // Debug
      const res = await axios.patch(`/api/admin/auth/combo-offers/${offerId}/image/${index}/toggle`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log('Image toggle response:', res.data); // Debug
      setComboOffers(comboOffers.map((offer) => (offer._id === offerId ? res.data.comboOffer : offer)));
      setFilteredComboOffers(filteredComboOffers.map((offer) => (offer._id === offerId ? res.data.comboOffer : offer)));
  ...
  if (!comboOffers || !products) {
    ...
    return <div className="text-red-600">Error: Invalid combo offer or product data</div>;
  }

  return (
    <div>
      {parentLoading || isAdmin === null ? (
          ...
          <div className="bg-white p-6 rounded-2xl shadow-md mb-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
              <form onSubmit={(e) => {
                ...
                axios.get(`/api/admin/auth/combo-offers/search?name=${encodeURIComponent(searchQuery)}`)
                  .then((response) => {
