...
const LoginRegister = () => {
  ...
  const handleApiCall = useCallback(
    async (callback, successMessage) => {
      ...
      try {
        ...
        toast.success(res.data.message || successMessage);
        if (res.data.token) {
          console.log('Token set:', res.data.token); // Debug token
          localStorage.setItem('token', res.data.token);
          try {
  ...
  const handleLoginRegister = useCallback(() => {
    ...
    if (pinString.length !== 6) return toast.error('Please enter a 6-digit PIN');
    handleApiCall(
      () =>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-100 via-green-200 to-green-100 px-4">
      <Toaster position="top-right" toastOptions={{ className: 'text-sm sm:text-base' }} />
      <div className="w-full max-w-md sm:max-w-lg bg-white rounded-3xl shadow-xl p-6 sm:p-8 border border-green-200 relative overflow-hidden">
        <div className="flex justify-center mb-2">
          <img
            src={agroLogo}
            alt="AgroTrade Logo"
        ...
        <div className="space-y-6">
          {!showPinInput ? (
            <div className="flex gap-2">
              <select
                className="w-18 border border-gray-300 rounded-lg px-3 py-3 text-base bg-white focus:ring-2 focus:ring-green-400 touch-none"
                value={countryCode}
                onChange={(e) => setCountryCode(e.target.value)}
              >
                {countryCodes.map((country) => (
              ...
              <input
                ...
                placeholder="Enter phone number"
                className="flex-1 border border-gray-300 rounded-lg px-3 py-3 text-base focus:ring-2 focus:ring-green-400 touch-none"
            ...
            <div className="flex justify-between gap-2">
              {pin.map((digit, index) => (
                <input
                  ...
                  maxLength={1}
                  className="w-12 sm:w-14 h-12 sm:h-14 border border-gray-300 rounded-lg text-center text-xl font-mono focus:ring-2 focus:ring-green-400 touch-none"
          ...
          <button
            ...
            className={`w-full ${
              loading
                ? 'bg-gray-400 cursor-not-allowed'
                : 'bg-emerald-500 hover:bg-emerald-600'
            } text-white font-semibold py-3 rounded-xl transition-all duration-300 shadow-md focus:outline-none focus:ring-2 focus:ring-green-400 touch-none`}
          >
