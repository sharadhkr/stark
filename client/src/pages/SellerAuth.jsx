...
const SellerAuth = () => {
  ...
  const validateForm = useCallback(
    (data = formData) => {
      ...
      if (isRegister) {
        ...
        if (!data.shopName) newErrors.shopName = 'Shop name is required';
        if (!data.address) newErrors.address = 'Address is required';
        if (data.otp.join('').length !== 6) newErrors.otp = 'Enter a 6-digit OTP';
        if (!image) newErrors.image = 'Profile picture is required';
      }
      if (!data.password || data.password.length < 6) newErrors.password = 'Password must be 6+ characters';
      return newErrors;
    },
    [formData, isRegister, image]
  );
  ...
  const handleSendOtp = useCallback(() => {
    ...
    setOtpSent(true);
    toast.success('OTP simulation enabled. Enter any 6-digit number.');
  }, [formData.phoneNumber, validateForm]);
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      const endpoint = isRegister ? '/api/seller/auth/temp-register' : '/api/seller/auth/login';
      let submitData;
      if (isRegister) {
        const formDataToSend = new FormData();
        formDataToSend.append('phoneNumber', `${countryCode}${formData.phoneNumber}`);
      ...
      try {
        ...
        toast.dismiss(loadingToast);
        toast.error(error.response?.data?.message || 'Registration/Login failed');
      } finally {
        setLoading((prev) => ({ ...prev, submit: false }));
      }
    },
    [formData, image, isRegister, storeToken, navigate, validateForm]
  );
  ...
  return <div><div className="min-h-screen bg-gradient-to-tr from-green-100 via-white to-teal-100 flex items-center justify-center relative overflow-hidden">
      <Toaster position="top-center" />
      <div className="absolute inset-0 bg-cover bg-center opacity-10" style={{ backgroundImage: `url(${bgImage})` }}></div>
      <motion.div className="z-10 w-full max-w-md bg-white/90 backdrop-blur-md shadow-2xl rounded-3xl p-8 border border-teal-200">
        <div className="flex justify-center mb-6">
          <img src={logo} alt="Logo" className="h-16 w-16" />
        </div>
        <h2 className="text-center text-xl font-semibold text-teal-700 mb-4">
          {isRegister ? 'Register as Seller' : 'Seller Login'}
        </h2>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="text-sm">Phone Number</label>
            <div className="flex gap-2">
              <span className="bg-teal-100 px-3 py-2 rounded-lg">+91</span>
              <input
                type="tel"
                name="phoneNumber"
                value={formData.phoneNumber}
          ...
          {isRegister && otpSent && (
            <div>
              ...
              {resendTimer > 0 ? (
                <p className="text-xs text-gray-500 mt-1">Resend in {resendTimer}s</p>
              ) : (
                <button type="button" onClick={handleSendOtp} className="text-xs text-teal-600 mt-1">
                  Resend OTP
                </button>
              )}
          ...
          {isRegister && (
            <>
              <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Full Name" className="w-full border rounded-lg px-3 py-2" />
              <input type="text" name="shopName" value={formData.shopName} onChange={handleChange} placeholder="Shop Name" className="w-full border rounded-lg px-3 py-2" />
              <textarea name="address" value={formData.address} onChange={handleChange} placeholder="Address" className="w-full border rounded-lg px-3 py-2" />
              <input type="file" accept="image/*" onChange={handleImageChange} className="w-full" />
              {imagePreview && <img src={imagePreview} alt="Preview" className="w-12 h-12 rounded-full mt-2" />}
            </>
          )}
          ...
          <div>
            ...
            />
            <button type="button" onClick={() => setShowPassword((v) => !v)} className="text-xs text-teal-600 mt-1">
          ...
          <button type="submit" className="w-full py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
            {loading.submit ? 'Please wait...' : isRegister ? 'Register' : 'Login'}
          </button>
        </form>
        <p className="text-center text-sm mt-4">
          {isRegister ? 'Already a seller?' : 'New seller?'}{' '}
          <button className="text-teal-600 font-medium" onClick={toggleForm}>
            {isRegister ? 'Login' : 'Register'}
          </button>
        </p>
      </motion.div>
