...
//           axios.get(`/api/user/auth/products/${productId}`, { headers: { Authorization: `Bearer ${token}` } }),
...
//   if (loading) {
//     return (
//       <div className="min-h-screen bg-gray-100 px-4 pt-6 pb-8">
...
//           <motion.button
//             onClick={() => navigate(-1)}
//             className="flex items-center gap-2 text-gray-700 hover:text-gray-900 font-medium bg-white px-4 py-2 rounded-xl shadow-md border border-gray-200"
...
//                 className={`flex-1 bg-gradient-to-r from-blue-500 to-teal-500 text-white py-3 rounded-xl font-medium shadow-md ${product.quantity === 0 ? 'bg-gray-400 cursor-not-allowed' : 'hover:from-blue-600 hover:to-teal-600'}`}
...
//                 className={`flex-1 bg-gradient-to-r from-yellow-400 to-orange-400 text-white py-3 rounded-xl font-medium shadow-md flex items-center justify-center gap-2 ${product.quantity === 0 ? 'bg-gray-400 cursor-not-allowed' : 'hover:from-yellow-500 hover:to-orange-500'}`}
//                 disabled={product.quantity === 0}
//                 variants={buttonVariants}
//                 whileHover="hover"
//                 whileTap="tap"
...
//                 className={`p-3 bg-white border border-gray-200 rounded-xl shadow-md ${inWishlist ? 'text-red-500' : 'text-gray-600 hover:bg-gray-50'}`}
//                 variants={buttonVariants}
//                 whileHover="hover"
//                 whileTap="tap"
      ...
      try {
        ...
        if (token) {
          try {
            ...
            // Don't show toast for wishlist errors to avoid clutter for public users
          }
        }
      } catch (error) {
        console.error('Error fetching product data:', error);
